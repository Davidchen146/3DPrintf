...
const operations_research::MPVariable* MeshOperations::addVariable(const double &coefficient, const double &min_val, const double &max_val, const std::string &name) {
    const operations_research::MPVariable* new_var = _solver->MakeIntVar(min_val, max_val, name);
...
const operations_research::MPVariable* MeshOperations::addXORVariable(const operations_research::MPVariable* var_1, const operations_research::MPVariable* var_2, const double &coefficient, const std::string &name) {
    // Create the new XOR variable
    const operations_research::MPVariable* xor_var = _solver->MakeIntVar(0.0, 1.0, name);

    // Add the constraints that make this an XOR variable
    double infinity = _solver->solver_infinity();
    double neg_infinity = -infinity;
    std::vector<const operations_research::MPVariable*> variables {var_1, var_2, xor_var};
...
void MeshOperations::addConstraint(const std::vector<const operations_research::MPVariable*> &variables, const std::vector<double> &coefficients, const double &min_val, const double &max_val, const std::string &name) {
    operations_research::MPConstraint* constraint = _solver->MakeRowConstraint(min_val, max_val, name);
...
void MeshOperations::preprocessSolver() {
    _solver = operations_research::MPSolver::CreateSolver("SCIP");
    if (!_solver) {
